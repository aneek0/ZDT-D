...
fun MyVpnProgramScreen(
  ...
  Column(
    ...
    ProgramDescriptionHeader(
      programId = "myvpn",
      description = stringResource(R.string.myvpn_program_hint),
      isProfiles = true,
    )
...
private fun MyVpnCreateProfileDialog(
  ...
  StyledCreateProfileDialog(
    ...
    onDismiss = onDismiss,
    onCreate = onCreate,
    titleRes = R.string.myvpn_create_profile_title,
    rulesRes = R.string.myvpn_profile_name_rules,
    invalidNameRes = R.string.myvpn_profile_name_invalid,
    validator = { name -> myVpnProfileNameRegex.matches(name) },
  )
}

@Composable
private fun MyVpnProfileEnabledCard(
  checked: Boolean,
