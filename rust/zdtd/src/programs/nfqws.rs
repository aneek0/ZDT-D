...
pub fn active_path() -> PathBuf { PathBuf::from(NFQWS_ROOT).join("active.json") }
pub fn profile_root(profile: &str) -> PathBuf { PathBuf::from(NFQWS_ROOT).join(profile) }
...
let resolved_mobile = count_valid_uid_pairs(&out_mobile)?;
    ...
    if resolved_total == 0 && has_launch_marker {
        log::info!("nfqws: launch marker present for {}, starting without routing app UIDs", profile_dir.display());
    }
