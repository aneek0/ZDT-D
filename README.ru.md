...
## Чем этот форк отличается от оригинала (GAME-OVER-op/ZDT-D)

Это независимый личный форк. Он регулярно мержится из апстрима `main` (ветки
вроде `merge-upstream-390`, `upstream/ai-polish`), поэтому большинство
программ, модель маршрутизации, оболочка UI, экраны «мировая карта» / web-панель
/ captive-portal / скрытие приложения / бэкап / планшет-TV, поддержка arm32 и
набор стратегий `nfqws2` унаследованы от оригинального проекта, а не добавлены
здесь.

Реально **добавлено в этом форке** (199 коммитов, специфичных для форка, поверх
точки слияния) — это небольшой сфокусированный набор:

- **Встроенный blockcheck (авто-тест стратегий).** UI `diagnostics/blockcheck/`
  плюс нативный режим `nfqws-tester auto`, который сначала проверяет список
  хостов без стратегии (бейзлайн), а затем прогоняет каждый файл стратегии
  `nfqws`/`nfqws2`, показывая процент «открытых» хостов и вердикт `Works` /
  `Partial` / `Failed` / `No baseline block` (с функцией шаринга результатов).
  В апстриме этого нет.
- **UI blockcheck / внешний вид.** В форке добавлен Compose-экран
  `BlockcheckScreen` (старт/стоп, выбор файла хостов, живой список стратегий с
  прогресс-барами и чипами `Works` / `Partial` / `Failed`), выдержанный в
  общем стиле карточек/кнопок приложения. Главный экран, статистика, power-wave
  кнопка, construction studio и apps host уже есть в апстриме — новым здесь
  является только экран blockcheck.
- **AMOLED-тема (`AmberOLED`).** В форке добавлен отдельный режим темы `AMOLED`
  (`ui/theme/Theme.kt`, `ZdtdThemeMode.AMOLED`), выбираемый в настройках
  (`SettingsContent`). Он использует настоящий чёрный фон `#000000` для экономии
  заряда на AMOLED-панелях и янтарный акцент (`#FFB300`), чтобы не светить синим
  при слабом освещении. Режимы `system` / `light` / `dark` уже есть в апстриме —
  новым здесь является только режим AMOLED.
- **Выбор IP-сетов и хост-листов пользователем.** Карточка настройки стратегии
  позволяет выбрать доменные хост-листы и IP-сеты (`--hostlist`,
  `--hostlist-exclude`, `--ipset`, `--ipset-exclude`); демон вырезает эти
  аргументы из каждого блока `--new` и подставляет выбор пользователя, а
  blockcheck переиспользует хост-листы настроенного профиля.
- **Кастомные Lua-примитивы nfqws.** Добавлены `fakemultisplit.lua`,
  `fakemultidisorder.lua`, `zapret-multishake.lua` и `custom_funcs.lua` в
  `module_template/strategic/lua/` (в апстриме там только `zapret-sni.lua` и
  `zapret-wgobfs.lua`).
- **Импорт данных magisk-zapret2 для `nfqws2` «второй бан».** Добавлены
  хост-листы по сервисам / файлы `ipset-*` и бинарники fake TLS/QUIC/`syn`
  пакетов в `module_template/strategic/bin/` и `strategic/list/`, которых нет в
  апстриме.
- **Инструменты обслуживания хост-листов** в `scripts/lists/`: `dedup_check.py`
  (проверка дублей локально и против zapret2-апстрима + бейзлайн),
  `strategy_dedup.py` (повторяет вырезание выбора демона при ручном
  редактировании) и харнесс валидации. В апстриме этого нет.
- **Специфичные для форка правки сборки/CI** (fast-build workflow, авто-синк
  prebuilt-бинарей, проверки обновления модуля, указывающие на форк
  `aneek0/ZDT-D`) и мелкие фиксы вроде восстановления строк/хост-листов форка,
  потерянных при мержах апстрима, и ужесточения логики stop/cleanup в `zdtd`.

Если вам нужен официальный проект — он тут:
[github.com/GAME-OVER-op/ZDT-D](https://github.com/GAME-OVER-op/ZDT-D). Этот форк
**не** поддерживается, не одобряется и не аффилирован с оригинальным автором.
