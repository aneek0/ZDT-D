...
Это независимый личный форк. Базовая модель маршрутизации, программы и пакет
Magisk/KernelSU/APatch унаследованы от оригинального проекта, но поверх них
добавлены собственные инструменты и стратегии. Основные отличия от ветки
`main` апстрима:

- **Встроенный blockcheck (авто-тест стратегий).** Добавлен UI
  `diagnostics/blockcheck/` и нативный режим `nfqws-tester auto`, который
  сначала проверяет список хостов без стратегии (бейзлайн), а затем
  прогоняет каждый файл стратегии `nfqws`/`nfqws2`, показывая процент
  «открытых» хостов и вердикт `Works` / `Partial` / `Failed` / `No baseline block`.
- **Выбор IP-сетов и хост-листов пользователем.** В приложении есть карточка
  настройки стратегии, где пользователь выбирает доменные хост-листы и IP-сеты
  (`--hostlist`, `--hostlist-exclude`, `--ipset`, `--ipset-exclude`). Демон
  вырезает эти аргументы из каждого блока `--new` стратегии и подставляет выбор
  пользователя, то есть выбор принадлежит демону, а не жёстко задан в файле.
- **Импорт данных magisk-zapret2 для `nfqws2`.** Добавлен набор стратегий
  «второго бана» в стиле zapret2 и связанные хост-листы / IP-сеты (большие
  файлы `ipset-*`, хост-листы по сервисам и бинарники fake TLS/QUIC/`syn`
  пакетов в `module_template/strategic/bin/`).
- **Собственные Lua-скрипты.** Добавлены кастомные nfqws Lua-примитивы в
  `module_template/strategic/lua/` (например `fakemultisplit.lua`,
  `fakemultidisorder.lua`, `zapret-multishake.lua`, `zapret-sni.lua`,
  `zapret-wgobfs.lua`, `custom_funcs.lua`).
- **Инструменты обслуживания хост-листов.** Добавлена папка `scripts/lists/`
  с `dedup_check.py` (проверка дублей локально и против zapret2-апстрима +
  бейзлайн) и `strategy_dedup.py` (повторяет вырезание выбора демона при
  ручном редактировании файлов стратегий).

Эти изменения — личные удобства и со временем могут расходиться с апстримом.
Этот форк **не** поддерживается оригинальным автором.
